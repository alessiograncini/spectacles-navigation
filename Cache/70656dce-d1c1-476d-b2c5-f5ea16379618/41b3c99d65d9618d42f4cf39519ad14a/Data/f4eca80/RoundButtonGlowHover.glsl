...
SC_DECLARE_TEXTURE(glow_map); // Title: Glow Map	
...
#define Node1_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(glow_map, UVCoord, 0.0)
...
#define Node3_Is_Less( Input0, Input1, Output, Globals ) Output = vec4( ssSmaller( Input0.x, Input1 ), ssSmaller( Input0.y, Input1 ), ssSmaller( Input0.z, Input1 ), ssSmaller( Input0.w, Input1 ) )
void Node2_Discard( in float4 Input, in float Threshold, out float4 Output, ssGlobals Globals )
{ 
	Output = Input;
	#ifdef FRAGMENT_SHADER
...
void main() 
{
	...
	
	{
		...
		float4 Color_N1 = float4(0.0); Node1_Texture_2D_Sample( CoordsOut_N9, Color_N1, Globals );
		float4 Value_N5 = float4(0.0); Node5_Gradient( Color_N1.x, NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 0.0 ), Port_Value0_N005 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N005 ), NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 1.0 ), Port_Value1_N005 ), NF_PORT_CONSTANT( float( 0.76 ), Port_Position2_N005 ), NF_PORT_CONSTANT( float4( 1.0, 0.960784, 0.537255, 1.0 ), Port_Value2_N005 ), NF_PORT_CONSTANT( float4( 1.0, 1.0, 1.0, 1.0 ), Port_Value3_N005 ), Value_N5, Globals );
		float4 Output_N3 = float4(0.0); Node3_Is_Less( Value_N5, NF_PORT_CONSTANT( float( 0.005 ), Port_Input1_N003 ), Output_N3, Globals );
		float4 Output_N2 = float4(0.0); Node2_Discard( Value_N5, Output_N3.x, Output_N2, Globals );
