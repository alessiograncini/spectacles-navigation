...
uniform NF_PRECISION float4 Port_Value3_N030;
uniform NF_PRECISION float Port_Position4_N030;
uniform NF_PRECISION float4 Port_Value4_N030;
uniform NF_PRECISION float4 Port_Value5_N030;
uniform NF_PRECISION float4 Port_Input1_N000;
uniform NF_PRECISION float3 Port_Normal_N001;
uniform NF_PRECISION float Port_Exponent_N001;
uniform NF_PRECISION float Port_Intensity_N001;
#endif	
...
void Node30_Gradient( in float Ratio, in float4 Value0, in float Position1, in float4 Value1, in float Position2, in float4 Value2, in float Position3, in float4 Value3, in float Position4, in float4 Value4, in float4 Value5, out float4 Value, ssGlobals Globals )
{ 
	...
	{
		Value = mix( Value4, Value5, clamp( ( Ratio - Position4 ) / ( 1.0 - Position4 ), 0.0, 1.0 ) );
	}
}
void Node1_Rim( in float3 Normal, in float Exponent, in float Intensity, out float Rim, ssGlobals Globals )
{ 
...
void main() 
{
	...
	
	{
		...
		float4 Value_N30 = float4(0.0); Node30_Gradient( Export_N55.x, NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 1.0 ), Port_Value0_N030 ), NF_PORT_CONSTANT( float( 0.3 ), Port_Position1_N030 ), NF_PORT_CONSTANT( float4( 0.290196, 0.240192, 0.113725, 1.0 ), Port_Value1_N030 ), NF_PORT_CONSTANT( float( 0.5 ), Port_Position2_N030 ), NF_PORT_CONSTANT( float4( 0.658824, 0.54902, 0.219608, 1.0 ), Port_Value2_N030 ), NF_PORT_CONSTANT( float( 0.6 ), Port_Position3_N030 ), NF_PORT_CONSTANT( float4( 0.976471, 0.804898, 0.290196, 1.0 ), Port_Value3_N030 ), NF_PORT_CONSTANT( float( 0.7 ), Port_Position4_N030 ), NF_PORT_CONSTANT( float4( 1.0, 0.87451, 0.498039, 1.0 ), Port_Value4_N030 ), NF_PORT_CONSTANT( float4( 0.495079, 0.454398, 0.0933547, 1.0 ), Port_Value5_N030 ), Value_N30, Globals );
		float Rim_N1 = 0.0; Node1_Rim( NF_PORT_CONSTANT( float3( 0.0, 0.0, 1.0 ), Port_Normal_N001 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Exponent_N001 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Intensity_N001 ), Rim_N1, Globals );
		float4 Output_N0 = float4(0.0); Node0_Mix( Value_N30, NF_PORT_CONSTANT( float4( 1.0, 0.890196, 0.34902, 1.0 ), Port_Input1_N000 ), Rim_N1, Output_N0, Globals );
		float4 Output_N100 = float4(0.0); Node100_Add( Output_N2, Output_N0, Output_N100, Globals );
