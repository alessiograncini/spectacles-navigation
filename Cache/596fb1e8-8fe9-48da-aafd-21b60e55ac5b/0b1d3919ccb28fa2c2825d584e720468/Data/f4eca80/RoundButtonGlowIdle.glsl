...
#if defined( STUDIO )
uniform NF_PRECISION float4 Port_Value_N010;
...
void Node10_Float_Value( in float4 Value, out float4 Output, ssGlobals Globals )
{ 
	Output = Value + 0.001;
...
void main() 
{
	...
	{
		float4 Output_N10 = float4(0.0); Node10_Float_Value( NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 0.0 ), Port_Value_N010 ), Output_N10, Globals );
		float4 Output_N3 = float4(0.0); Node3_Is_Less( Output_N10, NF_PORT_CONSTANT( float( 0.005 ), Port_Input1_N003 ), Output_N3, Globals );
		float4 Output_N2 = float4(0.0); Node2_Discard( Output_N10, Output_N3.x, Output_N2, Globals );
