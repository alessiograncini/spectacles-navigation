...
SC_DECLARE_TEXTURE(Tweak_N25); //       Title: Logo texture
uniform NF_PRECISION                    float  Tweak_N66; // Title: Logo UV Aspect Ratio
uniform NF_PRECISION                    float  Tweak_N64; // Title: Logo Scale Value
...
uniform NF_PRECISION float2 Port_Offset_N062;
uniform NF_PRECISION float2 Port_Center_N060;
uniform NF_PRECISION float Port_Value2_N053;
#endif	
...
#define Node62_Offset_Coords( CoordsIn, Offset, CoordsOut, Globals ) CoordsOut = CoordsIn + Offset
void Node66_Float_Parameter( out float Output, ssGlobals Globals ) { Output = Tweak_N66; }
void Node64_Float_Parameter( out float Output, ssGlobals Globals ) { Output = Tweak_N64; }
#define Node68_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node69_Construct_Vector( Value1, Value2, Value, Globals ) Value.x = Value1; Value.y = Value2
#define Node60_Scale_Coords( CoordsIn, Scale, Center, CoordsOut, Globals ) CoordsOut.xy = ( CoordsIn.xy - Center ) * Scale + Center
#define Node52_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(Tweak_N25, UVCoord, 0.0)
void Node51_Split_Vector( in float4 Value, out float3 Value1, out float Value2, ssGlobals Globals )
{ 
	Value1 = Value.rgb;
	Value2 = Value.a;
}
#define Node59_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, float3(Input2) )
...
{ 
	if ( bool( Tweak_N37 ) )
	{
		...
		
		{
			...
			float2 CoordsOut_N3 = float2(0.0); Node3_Rotate_Coords( UVCoord_N11, NF_PORT_CONSTANT( float( 90.0 ), Port_Rotation_N003 ), NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N003 ), CoordsOut_N3, Globals );
			...
			float2 CoordsOut_N62 = float2(0.0); Node62_Offset_Coords( UVCoord_N11, NF_PORT_CONSTANT( float2( -0.05, 0.0 ), Port_Offset_N062 ), CoordsOut_N62, Globals );
			...
			float Output_N68 = 0.0; Node68_Multiply( Output_N66, Output_N64, Output_N68, Globals );
			float2 Value_N69 = float2(0.0); Node69_Construct_Vector( Output_N68, Output_N64, Value_N69, Globals );
			float2 CoordsOut_N60 = float2(0.0); Node60_Scale_Coords( CoordsOut_N62, Value_N69, NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N060 ), CoordsOut_N60, Globals );
			float4 Color_N52 = float4(0.0); Node52_Texture_2D_Sample( CoordsOut_N60, Color_N52, Globals );
			float3 Value1_N51 = float3(0.0); float Value2_N51 = 0.0; Node51_Split_Vector( Color_N52, Value1_N51, Value2_N51, Globals );
			float3 Output_N59 = float3(0.0); Node59_Mix( Value_N0.xyz, Value1_N51, Value2_N51, Output_N59, Globals );
			
			Value1 = float4( Output_N59.xyz, 0.0 );
		}
		Result = Value1;
	}
	else
...
#define Node53_Construct_Vector( Value1, Value2, Value, Globals ) Value.rgb = Value1; Value.a = Value2
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	
	{
		...
		float4 Output_N17 = float4(0.0); Node17_Add( Export_N40, Result_N55, Output_N17, Globals );
		...
		float4 Output_N21 = float4(0.0); Node21_Blend( Output_N17, Color_N20, Output_N21, Globals );
		float4 Value_N53 = float4(0.0); Node53_Construct_Vector( Output_N21.xyz, NF_PORT_CONSTANT( float( 1.0 ), Port_Value2_N053 ), Value_N53, Globals );
