...
uniform NF_PRECISION            float4 tintColor; // Title: tintColor
SC_DECLARE_TEXTURE(mainTex); // Title: mainTex
uniform NF_PRECISION            float  brightness; // Title: brightness
uniform NF_PRECISION            float  fadeLevel; // Title: fadeLevel	
...
void Node3_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = tintColor; }
#define Node6_Swizzle( Input, Output, Globals ) Output = float3( Input.x, Input.y, Input.z )
#define Node1_Texture_2D_Object_Parameter( Globals ) /*nothing*/
#define Node2_Surface_UV_Coord( UVCoord, Globals ) UVCoord = Globals.Surface_UVCoord0
#define Node0_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(mainTex, UVCoord, 0.0)
#define Node5_Swizzle( Input, Output, Globals ) Output = float3( Input.x, Input.y, Input.z )
#define Node7_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node8_Swizzle( Input, Output, Globals ) Output = Input.w
#define Node9_Swizzle( Input, Output, Globals ) Output = Input.w
void Node12_Float_Parameter( out float Output, ssGlobals Globals ) { Output = brightness; }
void Node13_Float_Parameter( out float Output, ssGlobals Globals ) { Output = fadeLevel; }
#define Node10_Multiply( Input0, Input1, Input2, Input3, Output, Globals ) Output = Input0 * Input1 * Input2 * Input3
#define Node11_Construct_Vector( Value1, Value2, Value, Globals ) Value.xyz = Value1; Value.w = Value2
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	{
		float4 Output_N3 = float4(0.0); Node3_Color_Parameter( Output_N3, Globals );
		float3 Output_N6 = float3(0.0); Node6_Swizzle( Output_N3.xyz, Output_N6, Globals );
		Node1_Texture_2D_Object_Parameter( Globals );
		float2 UVCoord_N2 = float2(0.0); Node2_Surface_UV_Coord( UVCoord_N2, Globals );
		float4 Color_N0 = float4(0.0); Node0_Texture_2D_Sample( UVCoord_N2, Color_N0, Globals );
		float3 Output_N5 = float3(0.0); Node5_Swizzle( Color_N0.xyz, Output_N5, Globals );
		float3 Output_N7 = float3(0.0); Node7_Multiply( Output_N6, Output_N5, Output_N7, Globals );
		float Output_N8 = 0.0; Node8_Swizzle( Output_N3, Output_N8, Globals );
		float Output_N9 = 0.0; Node9_Swizzle( Color_N0, Output_N9, Globals );
		...
		float Output_N10 = 0.0; Node10_Multiply( Output_N8, Output_N9, Output_N12, Output_N13, Output_N10, Globals );
		float4 Value_N11 = float4(0.0); Node11_Construct_Vector( Output_N7, Output_N10, Value_N11, Globals );
