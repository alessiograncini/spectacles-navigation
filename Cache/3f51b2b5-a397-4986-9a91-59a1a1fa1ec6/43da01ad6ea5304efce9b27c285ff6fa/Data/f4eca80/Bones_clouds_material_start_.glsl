...
uniform NF_PRECISION float4 Port_Input1_N002;
uniform NF_PRECISION float Port_Input2_N002;
...
#define Node2_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, float4(Input2) )
...
{ 
	...
	if ( ( Switch ) == 0.0 )
	{
		...
		
		{
			...
			float4 Output_N2 = float4(0.0); Node2_Mix( Output_N80, NF_PORT_CONSTANT( float4( 1.0, 1.0, 1.0, 1.0 ), Port_Input1_N002 ), NF_PORT_CONSTANT( float( 0.5 ), Port_Input2_N002 ), Output_N2, Globals );
			float4 Output_N0 = float4(0.0); Node0_Multiply( Output_N69, Output_N2, Output_N0, Globals );
