...
uniform NF_PRECISION float4 Port_Value0_N016;
uniform NF_PRECISION float Port_Position1_N016;
uniform NF_PRECISION float4 Port_Value1_N016;
uniform NF_PRECISION float Port_Position2_N016;
uniform NF_PRECISION float4 Port_Value2_N016;
uniform NF_PRECISION float Port_Position3_N016;
uniform NF_PRECISION float4 Port_Value3_N016;
uniform NF_PRECISION float4 Port_Value4_N016;
uniform NF_PRECISION float4 Port_Input1_N050;
uniform NF_PRECISION float4 Port_Input0_N048;
uniform NF_PRECISION float3 Port_Normal_N023;
uniform NF_PRECISION float Port_Exponent_N023;
uniform NF_PRECISION float Port_Intensity_N023;
uniform NF_PRECISION float4 Port_Input0_N020;
uniform NF_PRECISION float4 Port_Input1_N020;
uniform NF_PRECISION float2 Port_Scale_N019;
uniform NF_PRECISION float2 Port_Center_N019;
uniform NF_PRECISION float Port_Rotation_N035;
uniform NF_PRECISION float2 Port_Center_N035;
uniform NF_PRECISION float Port_Value0_N030;
uniform NF_PRECISION float Port_Position1_N030;
uniform NF_PRECISION float Port_Value1_N030;
uniform NF_PRECISION float Port_Position2_N030;
uniform NF_PRECISION float Port_Value2_N030;
uniform NF_PRECISION float Port_Position3_N030;
uniform NF_PRECISION float Port_Value3_N030;
uniform NF_PRECISION float Port_Value4_N030;
#endif	
...
void Node16_Gradient( in float Ratio, in float4 Value0, in float Position1, in float4 Value1, in float Position2, in float4 Value2, in float Position3, in float4 Value3, in float4 Value4, out float4 Value, ssGlobals Globals )
{ 
...
void Node23_Rim( in float3 Normal, in float Exponent, in float Intensity, out float Rim, ssGlobals Globals )
{ 
...
#define Node0_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(capsule_btn_refl_tex_1, UVCoord, 0.0)
#define Node20_Smoothstep( Input0, Input1, Input2, Output, Globals ) Output = smoothstep( Input0, Input1, Input2 )
#define Node48_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, float4(Input1), Input2 )
#define Node50_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, Input2 )
void Node35_Rotate_Coords( in float2 CoordsIn, in float Rotation, in float2 Center, out float2 CoordsOut, ssGlobals Globals )
{ 
...
void Node30_Gradient( in float Ratio, in float Value0, in float Position1, in float Value1, in float Position2, in float Value2, in float Position3, in float Value3, in float Value4, out float Value, ssGlobals Globals )
{ 
	Ratio = clamp( Ratio, 0.0, 1.0 );
...
void main() 
{
	...
	
	{
		...
		float4 Value_N16 = float4(0.0); Node16_Gradient( Export_N55.x, NF_PORT_CONSTANT( float4( 0.807843, 0.796078, 0.498039, 1.0 ), Port_Value0_N016 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N016 ), NF_PORT_CONSTANT( float4( 0.464378, 0.446357, 0.28629, 1.0 ), Port_Value1_N016 ), NF_PORT_CONSTANT( float( 0.31 ), Port_Position2_N016 ), NF_PORT_CONSTANT( float4( 0.0500038, 0.0500038, 0.0500038, 1.0 ), Port_Value2_N016 ), NF_PORT_CONSTANT( float( 0.54 ), Port_Position3_N016 ), NF_PORT_CONSTANT( float4( 0.289998, 0.289998, 0.289998, 1.0 ), Port_Value3_N016 ), NF_PORT_CONSTANT( float4( 0.480003, 0.480003, 0.480003, 1.0 ), Port_Value4_N016 ), Value_N16, Globals );
		float Rim_N23 = 0.0; Node23_Rim( NF_PORT_CONSTANT( float3( 0.0, 0.0, 1.0 ), Port_Normal_N023 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Exponent_N023 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Intensity_N023 ), Rim_N23, Globals );
		float2 UVCoord_N17 = float2(0.0); Node17_Surface_UV_Coord( UVCoord_N17, Globals );
		float2 CoordsOut_N19 = float2(0.0); Node19_Scale_Coords( UVCoord_N17, NF_PORT_CONSTANT( float2( 1.0, 2.0 ), Port_Scale_N019 ), NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N019 ), CoordsOut_N19, Globals );
		float4 Color_N0 = float4(0.0); Node0_Texture_2D_Sample( CoordsOut_N19, Color_N0, Globals );
		float4 Output_N20 = float4(0.0); Node20_Smoothstep( NF_PORT_CONSTANT( float4( 0.470588, 0.470588, 0.470588, 0.0 ), Port_Input0_N020 ), NF_PORT_CONSTANT( float4( 0.239216, 0.239216, 0.239216, 0.0 ), Port_Input1_N020 ), Color_N0, Output_N20, Globals );
		float4 Output_N48 = float4(0.0); Node48_Mix( NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 0.0 ), Port_Input0_N048 ), Rim_N23, Output_N20, Output_N48, Globals );
		float4 Output_N50 = float4(0.0); Node50_Mix( Value_N16, NF_PORT_CONSTANT( float4( 1.0, 0.809812, 0.239216, 1.0 ), Port_Input1_N050 ), Output_N48, Output_N50, Globals );
		float2 CoordsOut_N35 = float2(0.0); Node35_Rotate_Coords( CoordsOut_N19, NF_PORT_CONSTANT( float( -90.0 ), Port_Rotation_N035 ), NF_PORT_CONSTANT( float2( 0.4, 0.5 ), Port_Center_N035 ), CoordsOut_N35, Globals );
		float Value_N30 = 0.0; Node30_Gradient( CoordsOut_N35.x, NF_PORT_CONSTANT( float( 1.0 ), Port_Value0_N030 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N030 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value1_N030 ), NF_PORT_CONSTANT( float( 0.33 ), Port_Position2_N030 ), NF_PORT_CONSTANT( float( 0.2 ), Port_Value2_N030 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Position3_N030 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value3_N030 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value4_N030 ), Value_N30, Globals );
		float4 Value_N18 = float4(0.0); Node18_Construct_Vector( Output_N50.xyz, Value_N30, Value_N18, Globals );
