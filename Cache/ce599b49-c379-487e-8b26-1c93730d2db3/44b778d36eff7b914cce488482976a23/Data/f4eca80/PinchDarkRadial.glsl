...
uniform NF_PRECISION                    float4 Tweak_N81; // Title: Base Color
...
uniform NF_PRECISION float Port_Position1_N078;
...
#define Node77_Surface_UV_Coord( UVCoord, Globals ) UVCoord = Globals.Surface_UVCoord0
void Node79_Split_Vector( in float4 Value, out float Value1, out float Value2, out float Value3, out float Value4, ssGlobals Globals )
{ 
	Value1 = Value.x;
	Value2 = Value.y;
	Value3 = Value.z;
...
void Node81_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = Tweak_N81; }
void Node5_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = baseColor; }
void Node78_Gradient( in float Ratio, in float4 Value0, in float Position1, in float4 Value1, in float4 Value2, out float4 Value, ssGlobals Globals )
{ 
	...
	NF_PREVIEW_SAVE( Value, 78, false )
}
...
{ 
	if ( bool( ENABLE_RECOLOR ) )
	{
		...
		
		{
			...
			float2 UVCoord_N77 = float2(0.0); Node77_Surface_UV_Coord( UVCoord_N77, Globals );
			float Value1_N79 = 0.0; float Value2_N79 = 0.0; float Value3_N79 = 0.0; float Value4_N79 = 0.0; Node79_Split_Vector( float4( UVCoord_N77.xy, 0.0, 0.0 ), Value1_N79, Value2_N79, Value3_N79, Value4_N79, Globals );
			float4 Output_N81 = float4(0.0); Node81_Color_Parameter( Output_N81, Globals );
			float4 Output_N5 = float4(0.0); Node5_Color_Parameter( Output_N5, Globals );
			float4 Value_N78 = float4(0.0); Node78_Gradient( Value2_N79, Output_N81, NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N078 ), Output_N81, Output_N5, Value_N78, Globals );
			float4 Value_N384 = float4(0.0); Node384_Float_Import( Value_N78, Value_N384, Globals );
